...
class BBOPlanner():
    ...
    def compute_internal_points(self, num=10,use_surface_points=False):
        if use_surface_points:
            grid_x,grid_z = torch.meshgrid(torch.linspace(-self.box_size[0]/2.0,self.box_size[0]/2.0,num).to(self.device),
            ...
            grid_x,grid_y,grid_z = torch.meshgrid(torch.linspace(-self.box_size[0]/2.0,self.box_size[0]/2.0,num).to(self.device),
                                                torch.linspace(-self.box_size[1]/2.0,self.box_size[1]/2.0,num).to(self.device),
                                                torch.linspace(-self.box_size[2]/2.0,self.box_size[2]/2.0,num).to(self.device))

            iternal_points = torch.cat([grid_x.reshape(-1,1),grid_y.reshape(-1,1),grid_z.reshape(-1,1)],dim=1).float()
        iternal_points = torch.mm(iternal_points,torch.from_numpy(self.box_rotation).float().to(self.device).T) + torch.from_numpy(self.box_pos).float().to(self.device)
    ...
    def compute_contact_points(self):
        ...
        p_l = torch.mm(p_l, torch.from_numpy(self.box_rotation).float().to(self.device).T) + torch.from_numpy(self.box_pos).float().to(self.device)
        ...
        p_r = torch.mm(p_r, torch.from_numpy(self.box_rotation).float().to(self.device).T) + torch.from_numpy(self.box_pos).float().to(self.device)
    ...
    def joint_limits_cost(self,theta,theta_max,theta_min):
        ...
        # print(theta)
        cost = torch.sum((theta-theta_max).clamp(min=0)**2,dim=1) + torch.sum((theta_min-theta).clamp(min=0)**2,dim=1)
...
if __name__ == '__main__':
    ...
    mid_l = torch.tensor([ 0.42704887,  0.17838557,  0.10469598, -1.74670609, -0.05181788 , 2.16040988,-2.29006758]).reshape(-1,7).to(device)
