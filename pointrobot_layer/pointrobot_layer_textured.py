...
class PointRobotLayer(torch.nn.Module):
    def __init__(self, device='cpu', mesh_path = os.path.join(os.path.dirname(os.path.realpath(__file__)),"../collision_avoidance_example/panda_urdf/meshes/visual/*.obj")):
        ...
        self.device = device
        self.urdf_path = os.path.join(dir_path, 'point_robot.urdf')
        self.mesh_path = mesh_path
        self.chain = pk.build_serial_chain_from_urdf(open(self.urdf_path).read().encode(),"base_link_y").to(dtype = torch.float32,device = self.device)
    ...
    def get_transformations_each_link(self,pose, theta):
        transformations = list(self.forward_kinematics(theta).values())
        return transformations
...
if __name__ == "__main__":
    device = 'cuda'
    panda = PointRobotLayer(device).to(device)
    scene = trimesh.Scene()
