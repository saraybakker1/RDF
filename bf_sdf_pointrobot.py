...
class BPSDF():
    ...
    def get_whole_body_sdf_with_joints_grad_batch(self,x,pose,theta,model,used_links = [0,1,2,3,4,5,6,7,8], nr_joints=7):
        nr_links = nr_joints+1
        delta = 0.001
        B = theta.shape[0]
        theta = theta.unsqueeze(1)
        d_theta = (theta.expand(B,nr_joints,nr_joints)+ torch.eye(nr_joints,device=self.device).unsqueeze(0).expand(B,nr_joints,nr_joints)*delta).reshape(B,-1,nr_joints)
        theta = torch.cat([theta,d_theta],dim=1).reshape(B*nr_links,nr_joints)
        pose = pose.unsqueeze(1).expand(B,nr_links,4,4).reshape(B*nr_links,4,4)
        sdf,_ = self.get_whole_body_sdf_batch(x,pose,theta,model,use_derivative = False, used_links = used_links)
        sdf = sdf.reshape(B,nr_links,-1)
        d_sdf = (sdf[:,1:,:]-sdf[:,:1,:])/delta
        return sdf[:,0,:],d_sdf.transpose(1,2) #todo: original sdf[:,0,:],d_sdf.transpose(1,2)
